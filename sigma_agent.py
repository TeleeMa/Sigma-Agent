...
from rvt.utils.rvt_utils import world_info_from_env, gather_features
import torch.nn.functional as F
from rvt.utils.ddp_utils import all_gather_batch
...
class SigmaAgent:
    ...
    def compute_contrastive_loss(self, out, labels):
        img_feat = out['img_feat']
        img_feat = img_feat / img_feat.norm(dim=1, keepdim=True)
        lang_feat = out['lang_feat']
        lang_feat = lang_feat / lang_feat.norm(dim=1, keepdim=True)
        img_feat_tp1 = out['img_tp_feat']
        img_feat_tp1 = img_feat_tp1 / img_feat_tp1.norm(dim=1, keepdim=True)
        img_feat_post_ca = out['img_feat_post_ca']
        img_feat_post_ca = img_feat_post_ca / img_feat_post_ca.norm(dim=1, keepdim=True)

        logit_scale = out['logit_scale']

        # img_feat_all, lang_feat_all = all_gather_batch([img_feat, lang_feat])
        img_feat_all, lang_feat_all, img_feat_tp1_all, img_feat_post_ca_all = all_gather_batch([img_feat, lang_feat, img_feat_tp1, img_feat_post_ca])
        logits_per_image = logit_scale * img_feat @ lang_feat_all.t()
        logits_per_text = logit_scale * lang_feat @ img_feat_all.t()

        logits_per_img_tp1 = logit_scale * img_feat_post_ca @ img_feat_tp1_all.t()
        logits_per_tp1_img = logit_scale * img_feat_tp1 @ img_feat_post_ca_all.t()

        loss = (F.cross_entropy(logits_per_image, labels) + F.cross_entropy(logits_per_text, labels)) / 2 \
            + (F.cross_entropy(logits_per_img_tp1, labels) + F.cross_entropy(logits_per_tp1_img, labels)) / 2
        return loss
    ...
    def update(
        ...
        labels_index = replay_sample['lang_index']

        action_gripper_pose_tp1 = replay_sample["gripper_pose_tp1"][:, -1]
        ...
        with torch.no_grad():
            ...
            wpt_local = torch.cat(wpt_local, axis=0) ###4, 3
            ...
            obs_tp1, pcd_tp1 = peract_utils._preprocess_inputs_tp(replay_sample, self.cameras)
            pc_tp1, img_feat_tp1 = rvt_utils.get_pc_img_feat(
                obs_tp1,
                pcd_tp1,
            )

            if self._transform_augmentation and backprop:
                _, _, pc_tp1 = apply_se3_aug_con(
                    pcd=pc_tp1,
                    action_gripper_pose=action_gripper_pose_tp1,
                    bounds=torch.tensor(self.scene_bounds),
                    trans_aug_range=torch.tensor(self._transform_augmentation_xyz),
                    rot_aug_range=torch.tensor(self._transform_augmentation_rpy),
                )

            pc_tp1, img_feat_tp1 = rvt_utils.move_pc_in_bound(
                pc_tp1, img_feat_tp1, self.scene_bounds, no_op=not self.move_pc_in_bound
            )
        ...
        lang_token_batch = torch.cat([lang_token[p].unsqueeze(0) for p in labels_index], dim=0)
        out = self._network(
            pc=pc,
            img_feat=img_feat,
            pc_tp1 = pc_tp1,
            img_feat_tp1 = img_feat_tp1,
            proprio=proprio,
            lang_emb=lang_goal_embs,
            lang_token=lang_token_batch,
            img_aug=img_aug,
        )
        ...
        if backprop:
            ...
            labels = torch.arange(bs, device=self._device, dtype=torch.long)
            labels = labels + bs * rank
            clip_loss = self.compute_contrastive_loss(out, labels)
    ...
    def act(
        ...
        #lang_goal_embs_real = torch.rand_like(lang_goal_embs)
