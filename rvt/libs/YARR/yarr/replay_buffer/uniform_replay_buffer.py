...
class UniformReplayBuffer(ReplayBuffer):
    ...
    def __init__(self,
        ...
        self._task_index = {}
        self._num_tasks = 0
        self.lang_index = json.load(open('./weights/text_dict.json'))
        ...
        if start_index % self._replay_capacity < end_index % self._replay_capacity:
            for i in range(start_index, end_index):
                ...
                with open(join(task_replay_storage_folder, '%d.replay' % task_index), 'rb') as f:
                    #print(join(task_replay_storage_folder, '%d.replay' % task_index))
                    d = pickle.load(f)
                    for k, v in d.items():
        ...
        with self._lock:
            ...
            for batch_element, state_index in enumerate(indices):
                ...
                for element_array, element in zip(batch_arrays,
                    ...
                    elif element.name == 'lang_index':
                        lang = self._get_element_stack(
                                store['lang_goal'],
                                state_index, terminal_stack)
                        element_array[batch_element] = self.lang_index[lang[0][0]]
    ...
    def get_transition_elements(self, batch_size=None):
        ...
        transition_elements = [
            ...
            ReplayElement('lang_index', (batch_size,), np.int32),
        ]
...
class UniformReplayBuffer_final(ReplayBuffer):
    """A simple out-of-graph Replay Buffer.
    ...
    def __init__(self,
        ...
        self.lang_index = json.load(open('/home/RLBench/RVT_v1/rvt/text_dict.json'))
    ...
    def sample_transition_batch(self, batch_size=None, indices=None,
        ...
        with self._lock:
            ...
            for batch_element, state_index in enumerate(indices):
                ...
                final_state_index = next_state_index
                while True:
                    terminal_final = self.get_terminal_stack(final_state_index)
                    if terminal_final[-1] == -1:
                        break
                    else:
                        final_state_index = final_state_index + 1
                    ...
                    if element.is_observation:
                        if element.name.endswith('tp1'):
                            element_array[
                                batch_element] = self._get_element_stack(
                                store[element.name[:-4]],
                                final_state_index % self._replay_capacity,
                                terminal_final)
                        else:
                            element_array[
                                batch_element] = self._get_element_stack(
