...
def eval(
    ...
    log_dir=None,
    verbose=True,
    save_video=False, ##False
):
    agent.eval()
    if isinstance(agent, sigma_agent.SigmaAgent):
        agent.load_clip()
    ...
    if tasks[0] =="openvocab":
        tasks = OPENVOCAB_TASKS
        if verbose:
            print(f"evaluate on {len(tasks)} tasks: ", tasks)
    ...
    for task_id in range(num_tasks):
        ...
        for ep in range(start_episode, start_episode + eval_episodes):
            ...
            if verbose:
                ...
                # logger.info("Evaluating %s | Episode %d | Score: %f | Episode Length: %d | Lang Goal: %s", task_name, ep, reward, len(episode_rollout), lang_goal)
                ...
                if isinstance(summary, VideoSummary):
                    ...
                    if task_rewards[video_cnt] > 99:
                        video_path = os.path.join(
                            record_folder,
                            f"{task_name}_success_{i}.mp4",
                        )
                        video_success_cnt += 1
                    else:
                        video_path = os.path.join(
                            record_folder, f"{task_name}_fail_{i}.mp4"
                        )
                        video_fail_cnt += 1
                    video_cnt += 1
