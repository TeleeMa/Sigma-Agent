...
from mvq_former.mvq_former import MVQ_Former
import sigma_agent as sigma_agent
from sigma_agent import print_eval_log, print_loss_log
...
def train(agent, dataset, training_iterations, rank=0):
    ...
    for iteration in tqdm.tqdm(
        ...
        res = torch.load('weights/lang.pth', map_location='cpu')
        lang_token = res['lang_token'].to(agent._device)
        # lang_emb = res['lang_embs'].to(agent._device)
        update_args.update(
            {   
                "rank": rank,
                "replay_sample": batch,
...
def experiment(rank, cmd_args, devices, port):
    ...
    TRAIN_REPLAY_STORAGE_DIR = "/mnt/ssd1/telima/rvt_replay"
    ...
    if exp_cfg.agent == "our":
        ...
        mvqformer = MVQ_Former(
            renderer_device=device,
            **mvt_cfg,
        ).to(device)
        n_parameters = sum(p.numel() for p in mvqformer.parameters() if p.requires_grad)
        print('number of params (M): %.2f' % (n_parameters / 1.e6))
        if ddp:
            mvqformer = DDP(mvqformer, device_ids=[device])

        agent = sigma_agent.SigmaAgent(
            network=mvqformer,
