...
class MVQ_Former(nn.Module):
    def __init__(
        ...
        self.pos_encoding_tp = nn.Parameter(
            torch.randn(
                1,
                num_pe_token,
                self.input_dim_before_seq,
        ...
        self.fc_bef_cross_attn = DenseBlock(
            attn_dim,
            attn_dim,
            norm=None,
            activation=None,
        ...
        self.visual_proj = DenseBlock(
                512,
                512,
                norm=None,
                activation=activation)

        self.visual_proj_post_ca = DenseBlock(
                512,
                512,
                norm=None,
                activation=activation)
        ...
        self.visual_token = nn.Parameter(torch.zeros(1, 1, self.im_channels))
        #self.visual_proj = nn.Parameter(torch.empty(attn_dim, 1024))

        self.img_tp_proj = DenseBlock(
                512*5,
                512,
                norm=None,
                activation=activation)
    ...
    def forward_features(self, img, pos_encoding):
        ...
        imgx = self.fc_bef_attn(ins)

        imgx = imgx.reshape(bs * num_img, num_pat_img * num_pat_img, -1)
        for self_attn, self_ff in self.layers[: len(self.layers) // 4]:
            imgx = self_attn(imgx) + imgx
            imgx = self_ff(imgx) + imgx

        return imgx, ins_orig_shape, d0
    ...
    def forward(
        ...
        **kwargs,
        ...
        imgx, ins_orig_shape, d0 = self.forward_features(img, self.pos_encoding)

        if img_tp is not None:
            imgx_tp, _, _ = self.forward_features(img_tp, self.pos_encoding_tp)

            imgx_tp = torch.mean(imgx_tp, dim=1, keepdim=True)
            imgx_tp = imgx_tp.view(bs, num_img, -1).flatten(1)
            imgx_tp = self.img_tp_proj(imgx_tp)
        else:
            imgx_tp = None

        visual_token = self.visual_token.expand(bs, -1, -1)
        ...
        lx = l
        vx = self.fc_bef_attn_v(visual_token)
        
        imgx = imgx.view(bs, num_img * num_pat_img * num_pat_img, -1)
        x = torch.cat((vx, imgx), dim=1)
        for self_attn, self_ff in self.layers[len(self.layers) // 4 : len(self.layers) // 2]:
            x = self_attn(x) + x
            x = self_ff(x) + x

        img_feat = x[:, :1].clone().squeeze(1)
        img_feat = self.visual_proj(img_feat)
        if lang_token is not None:
            lang_feat = self.lang_proj(lang_token)
        else:
            lang_feat = None

        x = torch.cat((lx, x), dim=1)
        x = self.fc_bef_cross_attn(x)
        # cross attention
        ...
        img_feat_post_ca = x[:, :1].clone().squeeze(1)
        img_feat_post_ca = self.visual_proj_post_ca(img_feat_post_ca)
        x = x[:, 1:]
        ...
        out = {"trans": trans, "feat": feat, "img_feat": img_feat, "img_feat_post_ca": img_feat_post_ca, "logit_scale": self.logit_scale.exp(), 'img_tp_feat': imgx_tp, 'lang_feat': lang_feat}
        return out
